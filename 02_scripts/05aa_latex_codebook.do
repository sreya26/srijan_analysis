*! Author: Hemanshu Kumar, hemanshu.kumar@ashoka.edu.in
...
program define latex_codebook ;
	...
	* check that specified size is legal ;
	...
	if "`r(ndistinct)'" == "" local numlevels 0 ;
	* the above accommodates a change in the way the -distinct- command works in its version 1.4.1, 
	* where it does not define the scalar in situations where the var has all missing values ;
